...
public class Methods {
    ...
    public Methods(Disk disk, JTree tree) {
        this.disk = disk;
        this.tree = tree;
    }
    ...
    public void initializeJTreeNodes(DefaultMutableTreeNode root) {
        ...
        treeNode = new TreeNode("capibara", "png", false);
        tempFile = new DefaultMutableTreeNode(treeNode);
        tempFolder.add(tempFile);
        disk.addToDisk(treeNode);
        ...
        treeNode = new TreeNode("Main", "java", false);
        tempFile = new DefaultMutableTreeNode(treeNode);
        tempFolder.add(tempFile);
        disk.addToDisk(treeNode);

        treeNode = new TreeNode("Lab_Rab_3", "rar", false);
        tempFile = new DefaultMutableTreeNode(treeNode);
        tempFolder.add(tempFile);
        disk.addToDisk(treeNode);

        treeNode = new TreeNode("Отчет_БД_№3", "txt", false);
        tempFile = new DefaultMutableTreeNode(treeNode);
        tempFolder.add(tempFile);
        disk.addToDisk(treeNode);
        ...
        treeNode = new TreeNode("Shaman - 4am", "wav", false);
        tempFile = new DefaultMutableTreeNode(treeNode);
        tempFolder.add(tempFile);
        disk.addToDisk(treeNode);

        treeNode = new TreeNode("Ed Sheeran - Shape of You", "wav", false);
        tempFile = new DefaultMutableTreeNode(treeNode);
        tempFolder.add(tempFile);
        disk.addToDisk(treeNode);
    ...
    public TreeNode addToJTree( DefaultMutableTreeNode selectedNode, TreeNode file){
        ...
        if(selectedNode.isRoot() || ((TreeNode) selectedNode.getUserObject()).isFolder() ) {
            ...
            JOptionPane.showMessageDialog(disk, "Файл можно добавить только в папку","Ошибка", JOptionPane.ERROR_MESSAGE);
        }
        return null;
    }
